-- Etsy outreach: tables, indexes, FKs, check constraints, and the
-- per-page counter RPC. Mirrors the consolidated bundle in
-- drizzle/sql/_etsy_outreach_bundle.sql plus the follow-up patches
-- (`reached_out_at`, focal points, 'queued' status) so a fresh local
-- Supabase stack ends up at the same schema the remote project carries.
--
-- Idempotent: every statement uses IF NOT EXISTS or a DO block that
-- swallows duplicate_object, so re-applying is safe.
...
CREATE TABLE IF NOT EXISTS "etsy_preview_pages" (
  ...
  "lifestyle_focal_point" jsonb,
  "lifestyle_face_box"    jsonb,
  "detail_url"            text,
  "detail_status"         text DEFAULT 'pending' NOT NULL,
  "detail_error"          text,
  "detail_focal_point"    jsonb,
  "detail_face_box"       jsonb,
